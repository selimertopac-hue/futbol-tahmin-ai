...
st.set_page_config(page_title="UltraSkor Pro: Coupon Center", page_icon="📝", layout="wide")
...
if mod == "🏆 Onur Listesi":
    ...
    if g_l:
        ...
        # 2. SÜRPRİZ RADARI (Top 3 Sürpriz)
        ...
        with col2:
            ...
            for m in surprizler:
                st.markdown(f'<div class="coupon-item"><b>{m["l_ad"]}</b> | {m["homeTeam"]["shortName"]} - {m["awayTeam"]["shortName"]}<br><span style="color:#f85149;">Tahmin: {m["res"]["nexus"]} (Riskli)</span></div>', unsafe_allow_html=True)
            st.markdown('</div>', unsafe_allow_html=True)

        # 3. GOL FESTİVALİ (Top 3 xG)
        ...
        with col3:
            ...
            for m in festivaller:
                st.markdown(f'<div class="coupon-item"><b>{m["l_ad"]}</b> | {m["homeTeam"]["shortName"]} - {m["awayTeam"]["shortName"]}<br><span style="color:#58A6FF;">xG: {m["res"]["total_xg"]:.2f} (Bol Gol)</span></div>', unsafe_allow_html=True)
            st.markdown('</div>', unsafe_allow_html=True)
        st.markdown("---")
    ...
    for m in top_20:
        ...
        st.markdown(f"""<div class="match-card"><div class="rank-badge">🔥 %{m['puan']}</div><div style="font-size:0.8rem;">{m['l_ad']} - Hafta {m['matchday']}</div><div style="display: flex; justify-content: space-between; align-items: center; margin-top:10px;"><div style="text-align: center; width: 30%;"><img src="{m['homeTeam']['crest']}" width="35"><br><b>{m['homeTeam']['name']}</b></div><div style="width: 30%; text-align: center;">{m_p}</div><div style="text-align: center; width: 30%;"><img src="{m['awayTeam']['crest']}" width="35"><br><b>{m['awayTeam']['name']}</b></div></div><div style="display: flex; justify-content: space-around; margin-top: 15px;"><div class="prediction-box">🤖 STD<br><b>{res['std']}{c_std}</b></div><div class="prediction-box">🛡️ SPEC<br><b>{res['spec']}{c_spec}</b></div><div class="prediction-box">🔥 NEXUS<br><b>{res['nexus']}{c_nx}</b></div></div></div>""", unsafe_allow_html=True)
else:
    # LİG ODAKLI
