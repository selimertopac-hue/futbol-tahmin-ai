...
st.set_page_config(page_title="UltraSkor Pro: Form Radar", page_icon="📈", layout="wide")
...
st.markdown("""
    ...
    .form-dot { display: inline-block; width: 12px; height: 12px; border-radius: 50%; margin: 0 2px; }
...
def get_form_dots(team_name, matches):
    ...
    for m in finished:
        ...
        dots_html += f'<span class="form-dot form-{res}" title="{res}"></span>'
    return f'<div style="margin-top:5px;">{dots_html}</div>'
...
if mod == "Global AI":
                ...
                <div class="match-card">
                    ...
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-top:10px;">
                        <div style="text-align: center; width: 30%;">
                            <img src="{m['homeTeam']['crest']}" width="35"><br><b>{m['homeTeam']['name']}</b>
                            {get_form_dots(m['homeTeam']['name'], m['l_full'])}
                        </div>
                        <div style="width: 30%; text-align: center;">{m_sk}</div>
                        <div style="text-align: center; width: 30%;">
                            <img src="{m['awayTeam']['crest']}" width="35"><br><b>{m['awayTeam']['name']}</b>
                            {get_form_dots(m['awayTeam']['name'], m['l_full'])}
                        </div>
                    </div>
    ...
    if l_matches:
        ...
        h_s = st.sidebar.selectbox("📅 Hafta", h_liste, index=h_liste.index(g_h) if g_h in h_liste else 0)
