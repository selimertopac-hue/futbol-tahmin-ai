...
st.set_page_config(page_title="UltraSkor Pro: Verified Oracle", page_icon="🎯", layout="wide")
...
# --- 3. ANALİZ VE BAŞARI MOTORU (BOZULMADI) ---
...
def analiz_et(ev, dep, matches):
    try:
        ...
        note = f"⚽ xG: {ex+ax:.2f} | Taktiksel analiz tamamlandı."
    ...
    if simdi < hedef_tarih:
        ...
        for l_ad, l_data in all_d.items():
            ...
            for m in [x for x in matches if x['matchday'] == target_md]:
                ...
                if res:
                    p = res['ae_c'] if "AETHER" in filtre else (res['s_c'] if "Standart" in filtre else (res['sp_c'] if "Spektrum" in filtre else res['n_c']))
        ...
        with c3:
            ...
            with c2:
                h = check_hit(surprizler, "surpriz")
                st.markdown(f'<div class="editor-card"><div class="coupon-title">🕵️ SÜRPRİZ (AETHER) <span class="success-badge">{h}/3</span></div>', unsafe_allow_html=True)
                for m in surprizler: st.markdown(f'<div class="coupon-item"><b>{m["l_ad"]}</b> | {m["homeTeam"]["shortName"]}-{m["awayTeam"]["shortName"]}<br>Tahmin: {m["res"]["aether"]}</div>', unsafe_allow_html=True)
                st.markdown('</div>', unsafe_allow_html=True)
            festivaller = sorted(g_l, key=lambda x: x['res']['total_xg'], reverse=True)[:3]
            with c3:
            ...
            for m in sorted(g_l, key=lambda x: x['puan'], reverse=True)[:20]:
                ...
                st.markdown(f"""<div class="match-card">
                    ...
                    <div class="ai-insight">💡 <b>Aether Insight:</b> {res['note']}</div>
