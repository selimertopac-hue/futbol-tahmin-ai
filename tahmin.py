...
st.markdown("""
    ...
    .milli-ara-box { background: #1c2128; border: 1px solid #58A6FF; padding: 50px; border-radius: 15px; text-align: center; margin: 30px auto; max-width: 800px; }
...
def analiz_ve_yorum(ev, dep, matches):
    try:
        ...
        tx = ex + ax
        if tx > 3.3: note = "⚽ **Gol Festivali:** Analizimiz bu maçta filelerin sıkça havalanacağını öngörüyor."
        elif e_t > 1.25: note = f"🚀 **Hücum İvmesi:** {ev} son haftalarda bitiricilik oranını ciddi ölçüde artırdı."
...
# --- 4. ZAMAN & HAFTA ---
simdi = datetime.now()
site_aktif_h = ((simdi - SİTE_DOGUM_TARİHİ).days // 7) + 1
...
if mod == "Global AI":
    ...
    s_sec = st.sidebar.selectbox("📅 Sitemiz: Hafta", range(1, site_aktif_h + 2), index=site_aktif_h-1)
    st.title(f"🚀 {filtre} - {s_sec}. Hafta")

    # KRİTİK HAFTA KONTROLÜ (Gelecek hafta seçildiyse)
    if s_sec > site_aktif_h and not tahmin_acik_mi():
        st.markdown(f"""<div class="lock-box"><h2>🔒 Gelecek Hafta Bekleniyor</h2><p>{s_sec}. Hafta tahminleri Cuma 12:00'de yayınlanacaktır.</p><div style="font-size:2.5rem; font-weight:bold; margin-top:10px; font-family:monospace;">{geri_sayim()}</div></div>""", unsafe_allow_html=True)
    else:
        g_l = []
        with st.spinner("Avrupa ligleri analiz ediliyor..."):
            for l_ad, l_data in all_d.items():
                ...
                # Sitenin 1. haftası (20 Mart) API'deki 25. hafta civarıydı.
                # Lig bazlı aktif haftayı bulup offset ile s_sec'e göre hedefliyoruz
                biten_mds = [m['matchday'] for m in matches if m['status'] == 'FINISHED']
                lig_aktif_md = max(biten_mds) if biten_mds else 1
                
                # Arşiv mi yoksa güncel mi?
                # Eğer s_sec aktif haftadan küçükse, ligin o zamanki haftasını çekiyoruz.
                target_md = lig_aktif_md - (site_aktif_h - s_sec)
        ...
        # MİLLİ ARA KONTROLÜ (Eğer liste boşsa)
        if not g_l:
            st.markdown("""<div class="milli-ara-box"><h2>🏁 Milli Ara Radarı</h2><p>Şu an takip ettiğimiz liglerde milli maç arası nedeniyle lig maçı bulunmamaktadır.</p><p style="color:#8B949E; font-size:0.9rem;">Bu süreçte arşiv haftalarını gezerek geçmiş analizlerimizi inceleyebilirsiniz. Bir sonraki bülten liglerin dönüşüyle birlikte Cuma 12:00'de burada olacaktır.</p></div>""", unsafe_allow_html=True)
        else:
            # LİSTELEME BAŞLIYOR (Kupon Paneli ve Kartlar)
            ...
            for m in top_20:
                ...
                <div class="match-card">
                    ...
                    <div style="font-size:0.8rem; color:#8B949E;">{m['l_ad']} - Lig Haftası {m['matchday']}</div>
                    ...
                    <div class="ai-insight">💡 <b>AI Analiz Notu:</b> {res['note']}</div>
                </div>""", unsafe_allow_html=True)

elif mod == "🏆 Onur Listesi":
    st.title("🏆 Sitemizin Gurur Tablosu")
    c1, c2, c3 = st.columns(3)
    with c1: st.markdown('<div class="hall-card"><span style="color:#58A6FF;">🤖 Standart AI</span><br><b>%72 Başarı</b><br><small>Hafta 1</small></div>', unsafe_allow_html=True)
    with c2: st.markdown('<div class="hall-card"><span style="color:#58A6FF;">🛡️ Spektrum AI</span><br><b>%76 Başarı</b><br><small>Hafta 1</small></div>', unsafe_allow_html=True)
    with c3: st.markdown('<div class="hall-card"><span style="color:#58A6FF;">🔥 Nexus AI</span><br><b style="color:#238636;">%84 Başarı</b><br><small>Hafta 1</small></div>', unsafe_allow_html=True)
else:
    # LİG ODAKLI
    st.sidebar.info("Lig Odaklı mod aktif.")
