...
st.set_page_config(page_title="UltraSkor Pro: Grand Festival", page_icon="🎯", layout="wide")
...
if mod == "🏠 Canlı Skorlar":
    ...
    if not matches: st.info("Aktif maç bulunmuyor.")
        ...
        for m in matches:
            ...
            st.markdown(f'<div class="match-card"><div style="display:flex; justify-content:space-between; font-size:0.8rem;"><span>{m["competition"]["name"]}</span></div><div style="display: flex; justify-content: space-around; align-items: center; margin-top:10px;"><b>{m["homeTeam"]["name"]}</b> <span style="font-size:1.5rem; color:#3fb950; font-weight:bold;">{score}</span> <b>{m["awayTeam"]["name"]}</b></div></div>', unsafe_allow_html=True)
    ...
    if simdi < hedef_tarih:
        ...
        if g_l:
            ...
            with c1:
                h = chk(imzalar, "banko"); seal = '<div class="full-hit-seal">🏆 FULL HIT</div>' if h == 3 else ""
            ...
            # 2. SÜRPRİZ
            surp = sorted([x for x in g_l if winner(x['res']['aether']) != "1"], key=lambda x: x['puan'], reverse=True)[:3]
            with c2:
                h = chk(surp, "surpriz"); seal = '<div class="full-hit-seal">🔥 BOOM!</div>' if h >= 2 else ""
                ...
                for m in surp: st.markdown(f'<div class="coupon-item"><b>{m["l_ad"]}</b> | {m["homeTeam"]["shortName"]}-{m["awayTeam"]["shortName"]}<br>AETHER: {m["res"]["aether"]}</div>', unsafe_allow_html=True)
                st.markdown('</div>', unsafe_allow_html=True)
            # 3. ÜST
            fest = sorted(g_l, key=lambda x: x['res']['total_xg'], reverse=True)[:3]
            with c3:
                h = chk(fest, "ust"); seal = '<div class="full-hit-seal">⚽ GOAL FEST</div>' if h == 3 else ""
                ...
                for m in fest: st.markdown(f'<div class="coupon-item"><b>{m["l_ad"]}</b> | {m["homeTeam"]["shortName"]}-{m["awayTeam"]["shortName"]}<br>xG: {m["res"]["total_xg"]:.2f}</div>', unsafe_allow_html=True)
                st.markdown('</div>', unsafe_allow_html=True)
            ...
            for m in sorted(g_l, key=lambda x: x['puan'], reverse=True)[:20]:
                ...
                st.markdown(f"""<div class="match-card">
                    <div style="float:right;" class="status-tag">{icon}</div>
                    <div class="rank-badge">🔥 %{m['puan']}</div>
                    <div style="font-size:0.8rem; color:#8B949E;">{m['l_ad']} - Hafta {m['matchday']}</div>
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-top:10px;">
                        <div style="text-align: center; width: 33%;"><img src="{m['homeTeam']['crest']}" width="30"><br><b>{m['homeTeam']['name']}</b>{get_form_dots(m['homeTeam']['name'], m['l_full'])}</div>
                        <div style="width: 33%; text-align: center;">{m_sk}</div>
                        <div style="text-align: center; width: 33%;"><img src="{m['awayTeam']['crest']}" width="30"><br><b>{m['awayTeam']['name']}</b>{get_form_dots(m['awayTeam']['name'], m['l_full'])}</div>
                    </div>
