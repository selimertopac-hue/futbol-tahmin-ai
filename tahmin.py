...
def master_analiz_et(ev_ad, dep_ad, all_matches):
    try:
        ...
        def skor(ex, ax):
            ...
            matrix = np.outer(
                [poisson.pmf(i, ex) for i in range(6)],
...
def form_analiz(ev_ad, dep_ad, all_matches):
    try:
        ...
        def form(maclar, takim):
            ...
            return g/len(maclar), y/len(maclar)
...
def haftalik_basari(m_data):
    ...
    for h in haftalar:
        ...
        results.append({
            "Hafta": h,
            "Standart AI %": round(std_ok / total * 100, 1),
            "Spektrum AI %": round(spec_ok / total * 100, 1)
        })
