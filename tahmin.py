...
# SİTE MİLAT: 20 MART 2026
SİTE_DOGUM_TARİHİ = datetime(2026, 3, 20) 

st.set_page_config(page_title="UltraSkor Pro: Milestone", page_icon="🏆", layout="wide")
...
def analiz_et(ev_ad, dep_ad, all_matches):
    try:
        ...
        def get_stats(team, is_home):
            ...
            if t_df.empty: return l_e_o, l_d_o, 1.0
        ...
        e_g, e_y, _ = get_stats(ev_ad, True)
        d_g, d_y, _ = get_stats(dep_ad, False)
...
# --- 5. ZAMAN VE GERİ SAYIM ---
simdi = datetime.now()
gun_farki = (simdi - SİTE_DOGUM_TARİHİ).days
site_aktif_haftasi = (gun_farki // 7) + 1

def geri_sayim_goster():
    # Bu haftanın Cuması, saat 12:00
    hedef_cuma = simdi + timedelta(days=(4 - simdi.weekday()) % 7)
    hedef_zaman = hedef_cuma.replace(hour=12, minute=0, second=0, microsecond=0)
    
    # Eğer Cuma 12:00'yi geçtiysek haftaya Cuma'ya kur
    if simdi >= hedef_zaman:
        hedef_zaman += timedelta(days=7)
        
    kalan = hedef_zaman - simdi
    saat, mod = divmod(kalan.seconds, 3600)
    dakika, saniye = divmod(mod, 60)
    
    return f"{kalan.days} Gün {saat:02d}:{dakika:02d}:{saniye:02d}"
...
if filtre == "Lig Odaklı":
    ...
    if l_m:
        ...
        if h_secim > guncel_h and not tahminler_acik_mi():
            st.markdown(f"""<div class="lock-box">🔒 Tahminler Cuma 12:00'de yayınlanacaktır.<br>
            <div class="timer-txt">{geri_sayim_goster()}</div></div>""", unsafe_allow_html=True)
        else:
    ...
    if site_h_secim > site_aktif_haftasi and not tahminler_acik_mi():
        st.markdown(f"""<div class="lock-box">🔒 Sitemizin {site_h_secim}. hafta tahminleri Cuma 12:00'de yayınlanacaktır.<br>
        <div class="timer-txt">{geri_sayim_goster()}</div></div>""", unsafe_allow_html=True)
    else:
        global_list = []
        with st.spinner("Taranıyor..."):
            for l_ad, l_data_raw in all_data.items():
                ...
                if not l_m: continue
                biten_mds = [x['matchday'] for x in l_m if x['status'] == 'FINISHED']
                lig_aktif_h = max(biten_mds) if biten_mds else 1
        ...
        # BAŞARI ÖZETİ
        if biten > 0:
            oran = int((isabet/biten)*100)
            st.markdown(f'<div class="success-summary">📈 BAŞARI NOTU: Bu bültende {biten} maçtan {isabet} tanesini doğru bilerek %{oran} başarı yakaladık!</div>', unsafe_allow_html=True)
